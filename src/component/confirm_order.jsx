...
function Confirm_Order({order}) {
  const handleCashOnDelivery = async () => {
    try {
      const response = await fetch('http://localhost:4000/cashondelivery/confirm_order', { // Corrected URL
        method: 'PUT',
        headers: {
  ...
  const handleOnlineCharge = async () => {
    try {
      const response = await fetch('http://localhost:4000/onlinecharge/confirm_order', { // Corrected URL
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
